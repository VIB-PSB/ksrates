...
def correct(config_file, trios_file):
    ...
    for __, row in trios.iterrows():
        ...
        if species_sister in db.index and species_out in db.index and sister_out in db.index:
            rate_species, rate_species_sd, rate_sister, rate_sister_sd = fcCorrect.compute_relative_rates(db, species_sister, species_out, sister_out, peak_stats)
            correct_peak, correct_sd = fcCorrect.compute_corrected_ks_species_sister(rate_species, rate_species_sd)
            # OC_segment is a measure of better/worse outgroup choices for the detection of relative rates through RRT; see documentation.
