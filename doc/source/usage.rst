...
Any time that the ortholog *K*:sub:`S` estimates and the related ortholog *K*:sub:`S` mode are estimated for a species pair, a local database is updated with such ortholog data (see ``ortholog_peak_db.tsv`` and ``ortholog_ks_list_db.tsv`` in :ref:`other_output`). Subsequent runs will skip the ortholog *K*:sub:`S` calculations for any species pair for which the ortholog *K*:sub:`S` mode has already been stored in the local database. The database is by default generated/accessed in the launching directory, but a custom path location can be specified in the :ref:`pipeline_config_section`.

In case a user doesn't want to reuse an existing ortholog *K*:sub:`S` mode of a particular species pair and wants instead to re-estimate it from the same input data but using e.g. a different number of bootstrap iterations or KDE bandwidth, the already calculated mode has to be manually deleted from the ``ortholog_peak_db.tsv`` database. The successive *ksrates* pipeline will re-estimate the mode according to the new parameters by starting from the previously computed ortholog *K*:sub:`S` estimates for the species pair concerned, thereby skipping the onerous ortholog *K*:sub:`S` estimation step.
