...
def run_phyml(msa, phyml_path='phyml'):
    ...
    # Using for-loops because glob returns a list (might be with more than one element?)
    for phyml_stats_file in glob.glob('{}_phyml_stats*'.format(msa_phyml)):
        os.remove(phyml_stats_file)
    ### os.system(' '.join(['rm', msa_phyml, '{}_phyml_stats*'.format(msa_phyml)]))
    for phyml_tree_file in glob.glob('{}_phyml_tree*'.format(msa_phyml)):
        shutil.move(phyml_tree_file, tree_path)
